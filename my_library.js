...
let songContainer = document.getElementById("library_songs");
...
let librarySongs = [1, 3, 5, 7, 8]; // User’s library
...
function nextSong() {
  ...
  if (isShuffling) {
    currentSongIndex = Math.floor(Math.random() * librarySongs.length);
  } else {
    currentSongIndex = (currentSongIndex + 1) % librarySongs.length;
  }
  loadSong(librarySongs[currentSongIndex]);
...
function prevSong() {
  if (librarySongs.length === 0) return;
  currentSongIndex = (currentSongIndex - 1 + librarySongs.length) % librarySongs.length;
  loadSong(librarySongs[currentSongIndex]);
  if (isPlaying) music.play();
  wave.classList.add("active1");
  playBtn.classList.remove("fa-play");
...
function playSong(id) {
  currentSongIndex = librarySongs.indexOf(parseInt(id));
  loadSong(id);
  music.play();
  isPlaying = true;
  playBtn.classList.remove("fa-play");
