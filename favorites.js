...
let songContainer = document.getElementById("favorite_songs");
...
function nextSong() {
  ...
  if (isShuffling) {
    currentSongIndex = Math.floor(Math.random() * favoriteSongs.length);
  } else {
    currentSongIndex = (currentSongIndex + 1) % favoriteSongs.length;
  }
  loadSong(favoriteSongs[currentSongIndex]);
...
function prevSong() {
  if (favoriteSongs.length === 0) return;
  currentSongIndex = (currentSongIndex - 1 + favoriteSongs.length) % favoriteSongs.length;
  loadSong(favoriteSongs[currentSongIndex]);
  if (isPlaying) music.play();
  wave.classList.add("active1");
  playBtn.classList.remove("fa-play");
...
function playSong(id) {
  currentSongIndex = favoriteSongs.indexOf(parseInt(id));
  if (currentSongIndex === -1) {
    currentSongIndex = 0;
    favoriteSongs.push(parseInt(id));
...
function renderSongs() {
  ...
  favoriteSongs.forEach((id, index) => {
    let song = songs.find(s => s.id === id);
    if (song) {
...
if (favoriteSongs.length > 0) {
  ...
  loadSong(8); // Default to "Lut Gaye" if no favorites
}
vol.value = 50;
setVolume();
renderSongs();